...
int main() {
    ...
    CreateFrames(&frame_to_send, 0, 0);
    if(sendto(sockfd, &frame_to_send, sizeof(frame), 0, (struct sockaddr*) &server_addr, sizeof(server_addr)) < 0) {
    ...
    if(recvfrom(sockfd, &frame_to_recv, sizeof(frame), 0, (struct sockaddr*)&server_addr, &len) < 0) {
        perror("Could not execute recvfrom either invalid socket, message to long, timeout or network error.\n");
        exit(EXIT_FAILURE);
    }
    else {

        if(checkFrames(&frame_to_recv, 1, 0) == 1)  {
            printf("Received SYN-ACK\n");
            CreateFrames(&frame_to_send, 2, 0);
        }
    ...
    printf("---------------------------------------\nSLIDING WINDOW\n---------------------------------------\n");
    /*int seqStart = 0;
    int track_current_start = 0;

    track_info frame_info;
    frame_info.frame_sent_count = 0;
    frame_info.seq = seqStart;
    frame_info.windowsize = WINDOWSIZE;
    frame_info.sock = sockfd;
    frame_info.sockaddr = server_addr;
    frame_info.size_addr = sizeof(server_addr);*/

    printf("---------------------------------------\nINITIALIZING CONNECTION TEARDOWN\n---------------------------------------\n");

    CreateFrames(&frame_to_send, 3, 0);
    if(sendto(sockfd, &frame_to_send, sizeof(frame), 0, (struct sockaddr*) &server_addr, sizeof(server_addr)) < 0) {
        perror("Could not execute sendto either invalid socket, connection lost or dest address is invalid.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if(checkFrames(&frame_to_recv, 4, 0) == 1)  {
